...
2. Every reference is rewritten — `new`, `::class`, type hints, imports, docblock type tags (`@param`, `@return`, `@var`) and the free-text documentation tags (`@see`, `@link`, `@uses`, including inline `{@see …}`). The rules register their renames with Rector's rename collector; `RenameClassRector` and the `@see`/`@link`/`@uses` pass come from the package's own `config/config.php`. `HihahoSetList::NAMING` imports that config, so the set needs nothing else. Registering a rule directly with `->withRules([AddNotificationSuffixRector::class])` relies on Rector auto-including the config, which needs the optional `rector/extension-installer` plugin; without it the rule aborts the run with an explanatory message rather than renaming declarations and orphaning their references.
