...
2. Every reference is rewritten — `new`, `::class`, type hints, imports, docblock type tags (`@param`, `@return`, `@var`) and the free-text documentation tags (`@see`, `@link`, `@uses`, including inline `{@see …}`). The rules register their renames with Rector's rename collector; `RenameClassRector` and the `@see`/`@link`/`@uses` pass come from the package's own `config/config.php`, which Rector auto-includes via `rector/extension-installer` (a dependency of this package). So `->withRules([AddNotificationSuffixRector::class])` on its own is enough — and `HihahoSetList::NAMING` imports that config directly, so the set works even where the installer plugin is disallowed. If neither path applies, the rule aborts the run with an explanatory message rather than renaming declarations and orphaning their references.
